...
def SSHLogin(host, port, username, password):
    """
    Attempt SSH login to the specified host using paramiko.
    """
    try:
...
def TelnetLogin(host, port, username, password):
    """
    Attempt Telnet login to the specified host using telnetlib (slated for removal for Python version 3.13)
    """
    user = bytes(username + "\n", "utf-8")
    ...
    try:
        ...
        print("Telnet is not running or connection could not be established on %s:%s" % (
            host, port))
    except EOFError:
